...
// 友達申請を承認
async function approveFriendRequest(userId, requesterId) {
    const client = await createClient();
    try {
        // 友達申請を承認
        await client.query(`
...
// 友達申請を取得
async function getFriendRequest(userId, requesterId) {
    const client = await createClient();
    try {
        const result = await client.query(`
...
async function checkFriendshipExists(userId, friendId) {
    const client = await createClient();
    try {
        const result = await client.query(`
      SELECT * FROM friendships 
